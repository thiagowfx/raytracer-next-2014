...
bool Matte::operator==(const Matte& o) const {
  return Material::operator==(o) && *ambient_brdf == *(o.ambient_brdf) &&
...
RGBColor Matte::shade(ShadeRec& sr) {
  ...
  for (unsigned j = 0; j < num_lights; ++j) {
    /* Direção da onde a luz vem (tendência: apontar para *fora* do objeto!).
     * Isso explica a convenção da luz direcional de especificar o contrário. */
...
RGBColor Matte::get_color() const { return diffuse_brdf->get_cd(); }
}
